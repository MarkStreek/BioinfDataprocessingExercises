...
rule bowtie2_index:
    ...
    log:
        "logs/bowtie2/{accession}.log"
    message:
        "Building bowtie2 index for {input}"
    threads:
        config['threads']
    conda:
        "../../envs/ConvertBAM_SAM.yaml"
    shell:
        "bowtie2-build {input} {output} 2> {log} --threads {threads}"
...
rule bowtie2_align:
    ...
    log:
        "logs/bowtie2/{accession}.log"
    message:
        "Aligning input samples to {input.idx}"
    threads:
        config['threads']
    conda:
        "../../envs/ConvertBAM_SAM.yaml"
    shell:
        "bowtie2 --local --no-unal -x {input.idx} -1 {input.left} -2 {input.right} | "
        "samtools view -bS - | samtools sort -no -- > {output} 2> {log} --threads {threads}"
